...
var calendarJs = {
	...
	setCalCurrentDate : (thisMonth) => { // 현재 스케줄러 달 업데이트
		calendarJs.calCurrentDate = calendarJs.dateToStr8(thisMonth.getFullYear(),thisMonth.getMonth()+1,thisMonth.getDate());
	},
	...
	getTodayOpenTime : (now) => { // 날짜 포멧 변경 (HHMI)
	...
	printMyRsvtSchd : (schdList) => {
		...
		if (schdList == null || schdList.length == 0) {
			...
			const schdDt = schdInfo.schdDt ? schdInfo.schdDt.substring(0,4) + '.' + schdInfo.schdDt.substring(4,6) + '.' + schdInfo.schdDt.substring(6,8) : '';
			$printWrap.html('<b>신청일자 </b><a href="/counsel/rsvt_timetable.do?dt=' + schdInfo.schdDt + '"> [' + schdInfo.schdTpNm + '] ' + schdDt + ' ' + schdInfo.schdStHh + ':' + schdInfo.schdStMi + ' ~ ' + schdInfo.schdEndHh + ':' + schdInfo.schdEndMi + '</a>');
		}
	},
	getSchdMode : () => {
	...
	setEduApplyBtnState : ($select, btnType) => {
		const btnSelectCnt= $('#divTimeTable [name="btnApplyDate"], #divTimeTable [name="btnCancelDate"]').filter('.on').length;
		...
		if (btnSelectCnt > 0) {
			...
			if (btnType==='btnCancelDate'){
				...
				const $applyReason = $frmApply.find('[data-apply-section="apply-reason"]');
				const schdMode = calendarJs.getSchdMode();
				const schdYmd = $select.data('schd-dt') + '';
				const schdDt = schdYmd.substring(0,4) + '-' + schdYmd.substring(4,6) + '-' + schdYmd.substring(6,8);
				...
				if (~~$('tr[data-schd-tmlv="' + nextHh + '' +  nextMi + '"] [data-schd-dt="' + schdYmd + '"]').attr('data-type')!==1) {
		...
		$('#btnChangeState').parent('div').removeClass('btn_gray btn_pink').addClass(cssNm);
		...
		$.ajax({
			...
			success: function(res){
				if (res.resultCode==='success') {
					...
					if (cntFail > 0) {
						calendarJs.setFormState($btn, false, '신청이 불가능합니다.\n일정을 다시 확인해 주세요.');
					} else {
					...
					calendarJs.setFormState($btn, false, '처리중 오류가 발생했습니다. 잠시후 다시 시도해 주세요.');
				};
			},
			error: function(xhr, option, error){
		...
		$.ajax({
			...
			success: function(res){
				if (res.resultCode==='success') {
					...
					if (cntFail > 0) {
						calendarJs.setFormState($btn, false, '삭제가 불가능합니다.\n일정을 다시 확인해 주세요.');
					} else {
						calendarJs.setFormState($btn, false, '');
					}
		...
		*/
		...
		function renderCalender(thisMonth) {
			...
			const setCalDateAttr = (dt, day) => {
				return ' title="dates" data-cal-dt="' + calendarJs.dateToStr8(dt.getFullYear(), dt.getMonth(), day) + '" ';
			}
			...
			if(!$('.calendar .cal_wrap').hasClass('register')){	//교육/상담신청 캘린더
				...
				calendarJs.getNurseScheduleApi({
						...
						*/
						// 이번달 내 검색
						startDt: calendarJs.dateToStr8(thisMonth.getFullYear(), thisMonth.getMonth() + 1, 1),
						endDt: calendarJs.dateToStr8(thisMonth.getFullYear(), thisMonth.getMonth() + 1, nextDate),
						hcSchdYn : 'N',
						rsvtSchdYn : 'Y'
					}, (res) => {
						if (res && res.resultCode==='success'){
							...
							const strNowDt = calendarJs.dateToStr8(today.getFullYear(), today.getMonth() + 1, today.getDate());
							...
							for(const key in schdTime) {
								...
								const rsvtRate = maxTime[key] === 0 ? 0 : schdTime[key] / maxTime[key];
								let cssName = 'cir04';
								if (!$('.dates .day button[data-cal-dt="' + key + '"]').parents('.day').hasClass('disable')) {
								...
								$('.dates .day button[data-cal-dt="' + key + '"]').append('<div class="circle ' + cssName + '"><!-- 신청현황 --></div>');
							}
							$('.calendar .dates .day.disable button').on('click', function(){
								const dt = $(this).data('cal-dt');
								const limitDt = new Date(today.getFullYear(), today.getMonth(), today.getDate() + 30);
								const strLimitDt = calendarJs.dateToStr8(limitDt.getFullYear(), limitDt.getMonth() + 1, limitDt.getDate());

								if (dt < strNowDt) {
									alert('지난 날짜는 신청할 수 없습니다.');
									return;
								} else if (dt > strLimitDt) {
									alert('상담신청은 오늘 이후 30일까지만 가능합니다.\n원하실 경우 온라인문의를 통해 담당간호사에게 요청해주세요.');
									return;
								} else if ($(this).attr('data-rsvt-state')=='closed') {
									alert('마감되어 신청가능한 시간이 없습니다.\n다른 날짜를 선택해 주세요.');
									return;
								}

							});
							$('.calendar .dates .day:not(.disable) button').on('click', function(){
								const dt = $(this).data('cal-dt');
								if (dt) {
									// 상담가능일정관리 상세
									...
									window.location.href = (path ? path : '/product') + '/rsvt_timetable.do?dt=' + dt;
								}
							});
						}
				});
		...
		function renderTimeTable(thisMonth) {
			...
			$('#divTimeTable table').css('width','calc((24.75vw * '+ (nextDate+1) +') + 12px)');
			...
			for (var i=0; i < nextDate; i++) {
				const strCurDate = calendarJs.dateToStr8(currentYear, currentMonth + 1, i + 1);
				const weekday = ['일', '월', '화', '수', '목', '금', '토'];
				day = new Date(currentYear, currentMonth, i + 1);
				week = weekday[day.getDay()];
				theadCell.append('<th ' + (strCurDate ==  strNowDt? ' class="today"':'') + ' data-head-dt="' + strCurDate + '"><span>' + (i + 1) + ' (' + week + ')</span></th>');
				tbodyCell.each(function(){
					const tm = $(this).data('schd-tmlv') + '';
					const hour = tm.substring(0,2);
					const min = tm.substring(2,4);
					const applyDate = calendarJs.dateToStr10(currentYear, currentMonth + 1, i + 1).replace('-','.') + ' ' + hour + ':' + min + '~ ';

					$(this).append('<td><button type="button" name="btnApplyDate" data-schd-dt="' + strCurDate + '" data-apply-dt="' + applyDate + '" data-schd-st-hh="' + hour + '" data-schd-st-mi="' + min + '" data-type="0">'+ (hour + ':' + min)+'</button></td>');
				});
				if (week === '토') {
			...
			calendarJs.getNurseScheduleApi({
					...
					if (res && res.resultCode==='success'){
						...
						// 예약현황 출력
						calendarJs.printMyRsvtSchd(res.data.mySchdList);
						calendarJs.cntMySchdList = res.data.mySchdList === null ? null : res.data.mySchdList.length;
						// 교육 이수 여부
						...
						res.data.nrsSchdList.forEach(function(schdRow){
							tbodyCell.each(function(){
								...
								if (schdRow.schdDt < strNowDt || (schdRow.schdDt == strNowDt && tm < strOpenTm)) {
									// 과거
									btnType = 0;
								} else {
									if (eduSchd.length > 0 ) {
								...
								if (btnMyRsvt) {
									...
									const strTime = eduSchdInfo.schdStHh + ':' + eduSchdInfo.schdStMi + ' ~ ' + eduSchdInfo.schdEndHh + ':' + eduSchdInfo.schdEndMi;
									const startTime = ~~eduSchdInfo.schdStHh * 60 + ~~eduSchdInfo.schdStMi;
									const endTime = ~~eduSchdInfo.schdEndHh * 60 + ~~eduSchdInfo.schdEndMi;
									const diffTime = ((endTime - startTime) / 30); // 30분을 1타임으로 계산
									
									if (tm === eduSchdInfo.schdStHh + '' + eduSchdInfo.schdStMi) {
										const applyDate = calendarJs.dateToStr10(eduSchdInfo.schdDt.substring(0,4), eduSchdInfo.schdDt.substring(5,6), eduSchdInfo.schdDt.substring(6,8)).replace('-','.') + ' ' + strTime;
										$(this).find('[data-schd-dt="' + schdRow.schdDt + '"]').before('<a href="javascript:;"  name="btnCancelDate" class="scheduled" data-apply-dt="' + applyDate + '" data-schd-seq="' + eduSchdInfo.schdSeq + '" style="height:' + (diffTime * 100)+ '%;"><div class="time">['+ strTime +']</div><p>' + eduSchdInfo.schdTpNm + ' 신청 완료</p></a>');
									}
								} else {
									$(this).find('[data-schd-dt="' + schdRow.schdDt + '"]').attr('data-type', btnType);
								}
							});
						});
						...
						// 타임테이블 내부 버튼 클릭 시
						const $btnSchd = $('#divTimeTable [name="btnApplyDate"], #divTimeTable [name="btnCancelDate"]');
						$btnSchd.on('click', function() {
							...
							if (schdMode==='01') {
								// 교육일 경우 1시간후도 선택
								const $btnNextTime = $(this).parents('tr').next().find('[name="btnApplyDate"][data-schd-dt="' + schdDate + '"]');
								if ($(this).hasClass('on')) {
			...
			if (calendarJs.isTimeTableScroll === false) {
				...
				const posTarget = calendarJs.dateToStr8(date.getFullYear(), date.getMonth() + 1, date.getDate());
				const pos = $('table thead tr th[data-head-dt="' + posTarget + '"]').offset();
				$('#divTimeTable').animate({
					scrollLeft: pos ? pos.left : 0
				}, 200);
			}
		...
		if($('.register').length){
			...
			// 모바일 터치 이벤트 (스와이프)
			calBoard.addEventListener("touchstart", (e) => {
				x1 = e.touches[0].pageX; // 터치가 시작되는 위치 저장
				y1 = e.touches[0].pageY;
			});
			calBoard.addEventListener("touchend", (e) => {
				x2 = e.changedTouches[0].pageX; // 터치가 끝나는 위치 저장
				y2 = e.changedTouches[0].pageY;
				const angle = calendarJs.getAngle(x1, y1, x2, y2);
				if (angle >= -45 && angle < 45) {
					//console.log('left');
					$('#divTimeTable').animate({
						scrollLeft: $('#divTimeTable').scrollLeft() + ((x1 - x2) * sensitive)
					}, speed);
				} else if (angle >= 45 && angle <= 135) {
					//console.log('under');
					$('#divTimeTable').animate({
						scrollTop: $('#divTimeTable').scrollTop() + ((y1 - y2) * sensitive)
					}, speed);
				} else if (angle >= 135 && angle <= 180 || angle >= -180 && angle <= -135) {
					//console.log('right');
					$('#divTimeTable').animate({
						scrollLeft: $('#divTimeTable').scrollLeft() - ((x2 - x1) * sensitive)
					}, speed);
				} else {
					//console.log('top');
					$('#divTimeTable').animate({
						scrollTop: $('#divTimeTable').scrollTop() - ((y2 - y1) * sensitive)
					}, speed);
				}
			});
		}
...
var eduApply = {
	init: () => {
		...
		$('#divTimeTable tbody tr').each(function(i){
			$('#divTimeTable tbody tr').eq(i).children('td').eq(idx).css({'background':'#fff7f8'});
		});
